...
&#9674; _Conformance (Handling incorrect annotations):_ If an incorrect or inconsistent
...
&#9674; _Conformance (Unsupported `WebServiceFeatureAnnotation`):_ If an unrecongnized or
...
`jakarta.annotation.Resource` annotation in JSR-250 <<bib35>>.
...
&#9674; _Conformance (JSR-181 conformance):_ A JAX-WS 2.0 implementation MUST be conformant to
...
`jakarta.xml.ws.RespectBinding`. If a JAX-WS implementation encounters an
