...
&#9674; _Conformance (Provider implementation):_ A `Provider` based service endpoint
implementation MUST implement a typed `Provider` interface.

&#9674; _Conformance (WebServiceProvider annotation):_ A `Provider` based service endpoint
...
&#9674; _Conformance (Publishing over HTTP):_ If the `Binding` for an `Endpoint` is a SOAP (see
...
&#9674; _Conformance (WSDL Publishing):_ An `Endpoint` that uses the SOAP 1.1/HTTP binding (see
...
&#9674; _Conformance (Checking publishEndpoint Permission):_ When any of the `publish`
...
&#9674; _Conformance (`Endpoint’s W3CEndpointReference`):_ The returned `W3EndpointReference`
