...
model of the client or the server. Web Services for Jakarta EE defines two
methods of implementing a Web service. It requires the Jakarta XML Web Services 
Servlet container based Java class programming model for implementing Web services 
...
A Jakarta XML Web Services Service Implementation Bean runs in a web 
container and an EJB Service Implementation Bean always runs in an 
EJB container.
...
Managed Beans specification specifies the requirements for these container-managed 
...
in sections 5.3.2.1 and 5.3.2.2.
...
can be found in Jakarta Web Services Metadata specification. If member attributes 
of the annotation are not specified then server side deployment descriptors 
(see section 7.1) are used. The member attributes of the annotation can 
...
is used as defined in the Jakarta Enterprise Beans 4.0 specification.
...
annotation, it must follow the packaging rules detailed in section 5.4. 
...
A Stateless Session Bean, as defined by the Jakarta Enterprise Beans 4.0 specification, 
can be used to implement a Web service to be deployed in the EJB container. 
A Singleton Session Bean, as defined by the Jakarta Enterprise Beans 4.0 specification, 
...
The access rules are specified in the Jakarta Enterprise Beans 4.0 specification.
...
See section 5.3.2.1 and 5.3.2.2
...
methods are not required to throw jakarta.rmi.RemoteException. The
...
methods are not required to throw jakarta.rmi.RemoteException. The business 
...
callbacks. See Jakarta Enterprise Beans 4.0 specification for more details on this.
...
Jakarta Enterprise Beans 4.0 specification (EJB Core Contracts and Requirements).

====== Allowed access to container services

The Jakarta Enterprise Beans 4.0 specification (EJB Core Contracts and
...
[#anchor-46]
====== Web container programming model for Jakarta XML Web Services
...
binary data types is provided by JAXB which is the data binding for Jakarta XML 
Web Services. Jakarta XML Web Services provides the MIME processing required to 
enable JAXB to serialize and deserialize MIME based MTOM/XOP packages.
...
packaging rules follow those defined by the Jakarta Enterprise Beans 4.0
